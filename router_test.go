...
func TestRouter(t *testing.T) {
	resp := kellyFramwork("/a/:path", "/a/xxx/yyy", []string{}, func(c *Context) {
	...
	resp = kellyFramwork("/a/:path", "/1/2/3/a/xxx", []string{"/1", "/2", "/3"}, func(c *Context) {
		c.WriteString(http.StatusOK, "ok")
	})
	if resp.StatusCode != http.StatusOK {
		t.Errorf("StatusNotFound error, %v", resp.StatusCode)
	...
	resp = kellyFramwork("/:a/*path", "/v1/v2/v3/va/xxx/yyy/zzz", []string{"/:1", "/:2", "/:3"}, func(c *Context) {
